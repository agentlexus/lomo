########################## TCL Event Handlers ##########################
#
#  LOMO_FG300C.tcl - 5_axis_dual_table
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Tables.
#
#  Created by BalagurovAI @ Wednesday, August 26 2026, 15:58:02 +0300
...
#
  ...
  set mom_sys_alt_unit_post_name                "LOMO_FG300C__IN.pui"
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   # Spindle/coolant are turned off only in MOM_end_of_program (M9, M5),
   # matching the reference NC 2.mpf — no spindle_off here.
...
#=============================================================
  ...
  global mom_sys_machine_time mom_machine_time
   ...
   MOM_do_template comment_blk_6

   MOM_do_template start_of_path_2

   MOM_do_template comment_blk_7

   PB_CMD_output_operation_comment

   MOM_do_template comment_blk_9
   ...
   foreach tool $tool_list {
      ...
      if [info exists tool_data_buffer($tool,output)] {
         ...
         if { [regexp {^([^\s]+)\s+([^\s]+)\s+([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)} $tool_line match number name dia rad flute adj] } {
            ...
            append tool_list_output ";T${number}=${name} D=${dia} R=${rad} H${adj} D00"
         }
      }
...
# а PC и пути к файлам — PB_CMD_output_program_footer.
  ...
  global mom_logname mom_date mom_machine_time mom_dnc_program_name
   ...
   if {[info exists mom_machine_time] && $mom_machine_time != ""} {
      MOM_output_literal ";(Machine time: $mom_machine_time MIN)"
   }
}
#=============================================================
...
#     ;(C:\...\*.mpf)
  ...
  global env
   ...
   if {[info exists mom_part_name] && $mom_part_name != ""} {
      MOM_output_literal ";(D:\\$mom_part_name)"
   }

   # Путь к выходному NC
   if {[info exists mom_dnc_program_name] && $mom_dnc_program_name != ""} {
      MOM_output_literal ";(C:\\$mom_dnc_program_name)"
   }
}
#=============================================================
...
#=============================================================
   ...
   if [CMD_EXIST OUTPUT_MACRO] {
      OUTPUT_MACRO $text_string $suppress_seqno
   } else {
      if { $suppress_seqno } {
         MOM_suppress once N
         MOM_output_literal $text_string
